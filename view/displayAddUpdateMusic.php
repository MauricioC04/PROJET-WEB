    ...
    <div class="container border rounded" style="background-color: #eeeeee;padding: 20px;position: absolute;transform: translate(-50%, -50%);left: 50%;top: 50%;max-width: 540px;">
        <div class="row">
            <div class="col-md-12">
                ...
                <label class="customFileInput" style="margin-top: 1rem;"><input type="file" id="musicInput" style="display: none" oninput="checkMusicInput();" value="<?php if ($_GET['action'] == 'displayUpdateMusic') :?>alreadySet<?php endif ?>" name="musicInput" <?php if ($_GET['action'] == 'displayAddMusic') :?>required<?php endif ?>>Choisir une morceau:&nbsp;&nbsp;</label><span id="nameMusic"><?php if ($_GET['action'] == 'displayUpdateMusic') :?><?= @$infosMusic[0]['id'] ?><?= @$infosMusic[0]['title'] ?>.mp3<?php endif ?></span>
